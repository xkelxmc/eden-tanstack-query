...
`initialCursor` filter option is provided. A provided cursor is compared as one
complete value, while route input remains partially matchable. With
`exact: true`, omitting `initialCursor` targets only queries whose initial cursor
is the default `null`.
