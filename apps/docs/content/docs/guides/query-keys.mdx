...
`initialCursor` in the filter options to target one variant. Cursor values are
compared as complete values rather than partial object or array prefixes, while
route input keeps TanStack Query's normal partial matching. `exact: true` uses
the default `null` cursor when `initialCursor` is omitted.
