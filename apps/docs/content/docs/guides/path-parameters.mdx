...
For `/users/:id?`, use `id` without the `?` suffix when supplying a value. To omit the parameter, access the route directly:
...
The callable form still requires a value. Use the direct branch instead of passing `{}` or `undefined`. `ExtractPathParams<'/users/:id?'>` produces `'id'`, and `PathParamsToObject<'/users/:id?'>` produces `{ id?: string }`.
...
function FileViewer({ category, filename }: { category: string; filename: string }) {
  ...
  // Adjacent params are applied in route order
...
Path parameters are stored separately from request input in the query key. Their path position and application order are retained so query fields, duplicate names, and infinite-query cursors cannot overwrite path identity.
...
TypeScript ensures all path parameters are present. If you miss a parameter, you get a compile-time error:
...
// Both compile — params accept string | number
eden.users({ id: 123 }).get.queryOptions()
eden.users({ id: '123' }).get.queryOptions()
```

Parameter names are inferred from the route path. Every parameter accepts `string | number`; values are serialized into the URL, and the library normalizes them when building the query key, so numeric and string forms share one cache entry.

## Method-Name Collisions

A static URL segment named after an HTTP method remains navigable when it is also a procedure, such as `eden.account.delete.post` for `POST /account/delete`. The procedure itself stays a plain object, so path parameters immediately after that method-named segment are unsupported. Procedure and route-schema members, host probes, and inherited `Object` members are also reserved at that position. Use a different route shape or the Treaty client directly for those collisions.

Outside a method-procedure collision, host-probe names such as `toJSON` and `$$typeof` are regular path segments. `then` is reserved everywhere so the proxy cannot be mistaken for a promise.
