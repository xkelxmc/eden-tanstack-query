...
The union also includes Treaty's `EdenFetchError<503, Error>` transport failure.
Routes without a declared non-success response fall back to
`EdenFetchError<number, unknown>`. If a route declares a `503` response, narrowing
to `503` produces either its declared body or the transport `Error`.
