...
| `opts.initialCursor` | `TCursor \| null` | Initial cursor value. Defaults to `null` |
...
`initialCursor` used by `infiniteQueryOptions`; omitting it targets the default
`null` cursor.
...
eden.route.get.infiniteQueryFilter(
  input?,
  filters?: QueryFilters & { initialCursor?: TCursor | null }
): QueryFilters
```
...
default. Pass `initialCursor` to target one complete cursor value; object and
array cursors are not treated as partial prefixes. Route input still uses
TanStack Query's normal partial matching. With `exact: true`, an omitted
`initialCursor` defaults to `null`, matching `infiniteQueryKey(input)`.
...
| `infiniteQueryKey` | `(input?, keyOpts?) => DataTag<EdenQueryKey>` | Infinite query key |
