...
The filter intentionally omits the initial cursor, so it matches every
initial-cursor variant for the selected route input. Pass `initialCursor` in
the filter options to target one variant. With `exact: true`, an omitted
