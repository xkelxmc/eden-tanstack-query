...
| [`infiniteQueryOptions`](#infinitequeryoptions) | Creates options for `useInfiniteQuery` on supported non-union cursor routes |
| [`infiniteQueryKey`](#infinitequerykey) | Generates an infinite query key on supported non-union cursor routes |
| [`infiniteQueryFilter`](#infinitequeryfilter) | Creates an infinite-query filter on supported non-union cursor routes |
...
Automatically set and cannot be overridden: `queryKey`, `queryFn`, `queryHash`. You can configure `queryKeyHashFn`.
...
eden.route.post.mutationOptions({ request: true, ...opts }): MutationOptions
```
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `opts` | `EdenMutationOptionsIn` | TanStack Query mutation options, excluding reserved options, plus the proxy-only `request` flag |
| `opts.request` | `true \| false` | Defaults to `false`. Set to `true` for typed `{ body, query, headers }` mutation variables |

With `request: true`, each invocation supplies its own body, query values, and headers. Required route fields remain required, and lifecycle callbacks receive the complete variables object. Omitted or false `request` keeps body-only variables. See [per-invocation query and headers](/docs/guides/mutations#per-invocation-query-and-headers) for examples and omission rules.
...
Mutation inputs do not accept `eden.context` or `eden.abortOnUnmount`. Earlier types accepted these settings, but neither had any effect. Remove them from existing calls. TanStack Query lifecycle callbacks, the context returned by `onMutate`, and `meta` remain supported. The returned `eden` property contains route metadata.
...
By default, the filter matches both regular and infinite queries. With `exact: true`, it matches the regular query with the same input and path parameters. Use `infiniteQueryFilter` for exact infinite-query matching.
